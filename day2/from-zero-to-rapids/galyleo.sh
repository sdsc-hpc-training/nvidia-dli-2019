...
galyleo_launch() {
  ...
  if [[ -z "${workdir}" ]]; then
    workdir="$(ssh ${user}@${system} 'echo ${HOME}')"
  fi
  scp "${job_script}" "${user}@${system}:${workdir}"
  galyleo_output "Upload complete."
 
  galyleo_output "Submitting Jupyter batch job script to ${system} ..."
  slurm_job_id="$(ssh ${user}@${system} "cd ${workdir}; sbatch ${job_script} | grep -o '[[:digit:]]*'")" 
  galyleo_output "Batch job submitted. Batch job id: ${slurm_job_id}"

  while [[ "${slurm_job_state}" != 'RUNNING' ]]; do
    slurm_job_state="$(ssh ${user}@${system} "squeue -j ${slurm_job_id} --noheader --Format='state:.7'")"
  ...
  slurm_job_nodelist="$(ssh ${user}@${system} "squeue -j ${slurm_job_id} --noheader --Format='nodelist' | sed -e 's/[[:space:]]*$//'")"
